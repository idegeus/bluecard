...
abstract class GameRules {
  ...
  Map<String, dynamic> executeMove(
...
class GameEngine {
  ...
  void selectGame(GameType gameType, List<String> playerIds) {
    ...
    if (playerIds.length < _currentRules!.minPlayers) {
      throw Exception(
        'Minimaal ${_currentRules!.minPlayers} spelers nodig voor ${gameType.displayName}',
      );
    }
    if (playerIds.length > _currentRules!.maxPlayers) {
      throw Exception(
        'Maximaal ${_currentRules!.maxPlayers} spelers toegestaan voor ${gameType.displayName}',
      );
    }
...
/// Pesten spelregels
class ZweedsPestenRules extends GameRules {
  ZweedsPestenRules() : super(GameType.zweedsPesten);
